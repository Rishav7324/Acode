...
if [ "$INSTALLING" = true ]; then
    ...
    # Configure apt to work inside proot (no sandbox, no chroot)
    mkdir -p /etc/apt/apt.conf.d
    cat > /etc/apt/apt.conf.d/99-proot <<'APTCONF'
...
# Default prompt with fish-style path shortening
_shorten_path() {
    local path="$PWD"
...
command_not_found_handle() {
    ...
    if command -v apt-file > /dev/null 2>&1; then
        pkg=$(apt-file search --package-only "bin/$cmd" 2>/dev/null | head -n1)
    fi
...
# Source user configs AFTER defaults
